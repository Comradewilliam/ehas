...
// Fetch regions and districts for dropdowns
$regions = [];
...
$districts = [];
// This will be populated via AJAX, but we need it declared for the initial load if there are errors

// Fetch patient's current profile data
$stmt = $conn->prepare("SELECT u.username, u.email, u.phone, u.address, u.dob, u.gender, u.region_id, u.district_id 
                        FROM users u 
...
if (!$patient_data) {
    ...
    // If patient data is fetched, also fetch districts for their current region
...
// Handle AJAX request for districts (if this script is called directly via AJAX)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($username) || empty($email) || empty($phone) || empty($address) || empty($dob) || empty($gender) || empty($region_id) || empty($district_id)) {
        ...
        $stmt->bind_param('ssi', $username, $email, $patient_id);
        $stmt->execute();
        $stmt->store_result();
        if ($stmt->num_rows > 0) {
            ...
            if ($stmt) {
                ...
                if ($stmt->execute()) {
                    ...
                    $stmt_re_fetch = $conn->prepare("SELECT u.username, u.email, u.phone, u.address, u.dob, u.gender, u.region_id, u.district_id 
                                                    FROM users u 
                                                    WHERE u.id = ? AND u.role = 'patient'");
                    $stmt_re_fetch->bind_param('i', $patient_id);
                    $stmt_re_fetch->execute();
                    $patient_data = $stmt_re_fetch->get_result()->fetch_assoc();
                    $stmt_re_fetch->close();

                    // Update session name if username changed
                    $_SESSION['name'] = $patient_data['username'];
...
// Display form if patient data is available
if ($patient_data) {
?>
    <h2>My Profile</h2>
    ...
    <form method="post" action="<?= BASE_URL ?>patient/profile.php">
    ...
    <script>
        document.getElementById('region_id').addEventListener('change', function() {
            ...
            if (regionId) {
                fetch('<?= BASE_URL ?>patient/profile.php?region_id=' + regionId)
...
<?php 
    ...
    echo '<p style="text-align: center;"><a href="' . BASE_URL . 'patient/index.php">Back to Dashboard</a></p>';
}
